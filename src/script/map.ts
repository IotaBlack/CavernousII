...
type TileToTileMap = {
    [key in mapChar]?: mapChar;
};

type descriptorMod = (d: string, x: number, y: number) => string;
type classMappingType = { [key in mapChar]: [string, string, boolean?, descriptorMod?] };
...
function setMined(x: number, y: number, icon?: mapChar) {

    const minedMapping:TileToTileMap = {
        '¤': '*',
        '☼': '©',
        '#': '.',
        '♠': '.',
    ...
    let tile =
        ...
        minedMapping[old] ||
        old;
    zones[currentZone].map[y] = zones[currentZone].map[y].slice(0, x) + tile + zones[currentZone].map[y].slice(x + 1) as unknown as mapChar[];
    if (tile !== old) {
        mapDirt.push([x, y]);
    }
}
    ...
    if (zones[displayZone].mapLocations[y] && zones[displayZone].mapLocations[y][x]) {
        ...
        for (let classMappingEntry of Object.entries(classMapping)) {
            if (classMappingEntry[1][0] == type) {
                let type = getLocationType(getLocationTypeBySymbol(classMappingEntry[0] as mapChar) || '');
                if(type === undefined){
