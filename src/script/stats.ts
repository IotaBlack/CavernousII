...
class Stat<statName extends anyStatName> {
    ...
    update(forceIncreaseAtUpdate = false): void {
        ...
        if (this.name === "Mana") {
            ...
            Current: ${writeNumber(this.current, 2)} + ${writeNumber(
                this.current < 100 ? this.bonus : (this.current * (100 + this.bonus)) / 100 - this.current,
                2
            )}
            Click to load best grind route (projected +${writeNumber(grindRoute?.projectedGain || 0, 3)}) in ${writeNumber(
                (grindRoute?.totalTime ?? 0) / 1000 || 0,
                1
            )}s`;
        }
        this.dirty = false;
...
type anyStatName = "Mana" | "Mining" | "Woodcutting" | "Magic" | "Speed" | "Smithing" | "Runic Lore" | "Spellcraft" | "Combat" | "Gemcraft" | "Attack" | "Defense" | "Health";
