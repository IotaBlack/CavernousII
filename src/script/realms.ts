...
const verdantMapping:TileToTileMap = {
    "#": "♠", // Limestone -> Mushroom
...
function convertMapToVerdant(map:Zone["map"]): mapChar[][] {
    return map.map(row => [...row].map(cell => verdantMapping[cell] || cell).join("") as unknown as mapChar[]);
}
