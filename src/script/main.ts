...
function getLocationTypeBySymbol(symbol: LocationType["symbol"]) {
    let locationType = locationTypes.find(a => a.symbol == symbol)
    if(locationType === undefined) throw new Error(`No location type with symobol "${symbol}"`);

    //maybe return the actual LocationType rather than just the name?
...
function ensureLegalQueues() {
    for (let i = 0; i < queues.length; i++) {
        if (queues[i].some(q => q[0] == "Q" && getActionValue(q[0]) >= savedQueues.length)) {
            queues[i] = []; //this breaks by assigning a plain array rather than an ActionQueue
        }
    }
...
setInterval(function mainLoop() {
    ...
    if (settings.usingBankedTime && timeBanked > 0) {
        const speedMultiplier = 3 + mana.base ** 0.4;
