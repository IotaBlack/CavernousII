...
class Zone {
    ...
    public constructor(name: string, map: string[], goalReward: (() => void) | null = null) {
        ...
        this.map = map.map(row => row.split("") as unknown as mapChar[]);
    ...
    resetZone() {
        this.map = this.originalMap.slice() as unknown as mapChar[][];
        if (realms[currentRealm].name == "Verdant Realm") {
            // Visual changes
            this.map = convertMapToVerdant(this.map);
        }
        if (this.goalComplete) this.map = this.map.map(row => (row as unknown as string).replace("√", "#") as unknown as mapChar[]);
    ...
    sumRoute(route: ZoneRoute, actualRequirements: simpleStuffList, startDamage: number[]) {
    ...
    display() {
        ...
        if (this.routesChanged) {
            ...
            for (let i = 0; i < this.routes.length; i++) {
                ...
                routeNode.querySelector<HTMLElement>(".delete-route")!.onclick = this.deleteRoute.bind(this, i);
