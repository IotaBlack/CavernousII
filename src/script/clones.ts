...
class Clone {
    ...
    executeAction(time: number, action: QueueAction, actionIndex: number) {
        ...
        if (!hasOffset && this.currentCompletions !== null && this.currentCompletions < location.completions && location.temporaryPresent?.name != "Teleport") {
            ...
            this.currentProgress = 0;
            this.selectQueueAction(actionIndex, 100);
            this.addToTimeline(!hasOffset ? locationPresentAction : locationEnterAction, initialTime - time);
            return time;
        }
    ...
    static performActions(time: number) {
        ...
        while (maxTime) {
            ...
            const nextSingleActionTime = Math.min(...nextActionTimes);
            clones.filter((c, i) => nextActionTimes[i] == nextSingleActionTime).forEach(c => c.performSingleAction(nextSingleActionTime + 0.001));
            maxTime = Math.max(
                ...clones.map((e, i) => (!e.noActionsAvailable && e.damage != Infinity && nextActionTimes[i] < Infinity ? e.timeAvailable || 0 : 0))
            );
