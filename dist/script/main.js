...
function save() {
    ...
    const savedGrindRoutes = JSON.parse(JSON.stringify(grindRoutes, (key, value) => {
        if (key == "usedRoutes")
            return undefined;
        return value;
    }));
...
setInterval(function mainLoop() {
    ...
    while (timeAvailable > 0) {
        ...
        if (timeLeft == timeAvailable || timeLeft == MAX_TICK)
            break;
...
const keyFunctions = {
    ...
    "KeyN": () => {
        if (!document.querySelector("#stuff .spell"))
            return;
        switchRuneList();
    },
    "KeyG": () => {
...
setTimeout(() => {
    ...
    if (templateSelect === null)
        throw new Error("No select template found");
